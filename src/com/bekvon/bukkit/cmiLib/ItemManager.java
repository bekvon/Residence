...
//    HashMap<String, String> customNamesEn = new HashMap<String, String>();
//    HashMap<String, String> customNames = new HashMap<String, String>();

    public ItemManager(Residence plugin) {
	this.plugin = plugin;
    }
...
//	    load();
	...
	String subdata = null;
	if (name.contains(":")) {
	    CMIItemStack mat = byBukkitName.get(name);
	    if (mat != null)
		return mat;
...
//	LEGACY_COMMAND_CHAIN(211, null, null, "LEGACY_COMMAND_CHAIN", ""),
	...
	public static CMIMaterial get(String id) {
		...
		if (ids != null) {
		    for (CMIMaterial one : CMIMaterial.values()) {
			if (data == null && one.getId() == ids)
			    return one;
			if (data != null && one.getId() == ids && one.getLegacyData() == data)
			    return one;
		    }
		}
	    }
	...
	public static CMIMaterial get(Material mat) {
		...
		CMIMaterial m = byRealMaterial.get(mat);
		if (m != null)
		    return m;
	    }
	    return CMIMaterial.NONE;
...
//	    }
	    CMIMaterial m = null;
	    if (Version.isCurrentEqualOrLower(Version.v1_13_R2)) {
		m = Version.isCurrentEqualOrHigher(Version.v1_13_R1) ? get(item.getType().getId()) : get(item.getType().getId(), item.getData().getData());
		...
		m = byRealMaterial.get(item.getType());
	    }
	...
	public static CMIMaterial get(Block block) {
		...
		m = byRealMaterial.get(block.getType());
	    }
	    ...
	    if (m == null && Version.isCurrentEqualOrLower(Version.v1_13_R2)) {
		m = get(block.getType().getId(), Version.isCurrentEqualOrHigher(Version.v1_13_R1) ? 0 : data);
	    }
	    return m == null ? CMIMaterial.NONE : m;
	}

...
public class SelectionManager {
	...
	public void showSelectionInfo(Player player) {
		...
		if (this.hasPlacedBoth(pname)) {
			CuboidArea cuboidArea = new CuboidArea(getPlayerLoc1(pname), getPlayerLoc2(pname), null);
	...
	public void vert(Player player, boolean resadmin) {
		if(hasPlacedBoth(player.getName()) || worldEdit(player)) {
	...
	public void modify(Player player, boolean shift, int amount) {
		...
		CuboidArea area = new CuboidArea (playerLoc1.get(player.getName()),playerLoc2.get(player.getName()), null);
		if(d == Direction.UP) {

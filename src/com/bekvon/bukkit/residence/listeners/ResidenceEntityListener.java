...
public class ResidenceEntityListener{
	public ResidenceEntityListener(Plugin plugin) {
		if(Residence.getConfigManager().isEnderManBuildEnabled()){
			Residence.getServ().getPluginManager().registerEvents(new EnderManBuild(), plugin);
		}
		if(Residence.getConfigManager().isEntityInteractEnabled()){
			Residence.getServ().getPluginManager().registerEvents(new EntityInteract(), plugin);
		}
		if(Residence.getConfigManager().isCreatureSpawnEnabled()){
			Residence.getServ().getPluginManager().registerEvents(new CreatureSpawn(), plugin);
		}
		if(Residence.getConfigManager().isPaintingListenerEnabled()){
			Residence.getServ().getPluginManager().registerEvents(new PaintingListener(), plugin);
		}
		if(Residence.getConfigManager().isEntityCombustEnabled()){
			Residence.getServ().getPluginManager().registerEvents(new EntityCombust(), plugin);
		}
		if(Residence.getConfigManager().isExplosionEnabled()){
			Residence.getServ().getPluginManager().registerEvents(new ExplosionEvent(), plugin);
		}
		if(Residence.getConfigManager().isPvpListenerEnabled()){
			Residence.getServ().getPluginManager().registerEvents(new PVPListener(), plugin);
		}
	}
	public class EnderManBuild implements Listener{
	...
	public class PaintingListener implements Listener{
		...
		public void onPaintingPlace(PaintingPlaceEvent event) {
			...
			if(res!=null){
				...
				perms = new WorldPermissions(event.getPainting().getLocation().getWorld().getName());
			}
			String pname = player.getName();
			boolean hasplace = perms.playerHas(pname, "place", perms.playerHas(pname, "build", true));
			if (!hasplace) {
				event.setCancelled(true);
		...
		public void onEntityDamage(EntityDamageEvent event) {
			...
			if (event instanceof EntityDamageByEntityEvent) {
				...
				if ((ent instanceof Player || tamedWolf) && (damager instanceof Player || damager instanceof Arrow && ((Arrow)damager).getShooter() instanceof Player)) {
					...
					if (area == null) {
						/* World PvP */
						if (!new WorldPermissions(damager.getWorld().getName()).has("pvp", true)) {
					...
					if (area == null) {
						if (!new WorldPermissions(damager.getWorld().getName()).has("creeper", true)) {
							event.setCancelled(true);
						}
					} else {
			...
			if (area == null) {
				if (!new WorldPermissions(ent.getWorld().getName()).has("damage", true) && (ent instanceof Player || tamedWolf)) {
					event.setCancelled(true);
				}
			} else {
